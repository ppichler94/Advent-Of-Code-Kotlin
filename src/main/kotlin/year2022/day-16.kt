...
open class PartA16 : Part() {
    ...
    override fun parse(text: String) {
        ...
        distances =
            significantValves.associate { from ->
                val traversal =
                    TraversalBreadthFirstSearch { n, _ -> valveNeighbours(n) }
                        .startFrom(from.name)
                        .traverseAll()
                from.name to
                    significantValves
                        .filter { it != from && it.name != "AA" }
    ...
    override fun compute(): String = traverse(timeLeft = 30).toString()
...
class PartB16 : PartA16() {
    override fun compute(): String = traverse(timeLeft = 26, withElephant = true).toString()
