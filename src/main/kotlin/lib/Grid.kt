...
sealed class BaseGrid2d<T>(private val content: Iterable<Iterable<T>>) {
     ...
     */
    fun findAll(elements: Iterable<T>) = buildList {
 ...
 */
data class Position(val position: Vector) {
    companion object {
        fun at(vararg coordinates: Int): Position {
            return Position(Vector(coordinates))
        }
         ...
         */
        fun moves(dimensions: Int = 2, diagonals: Boolean = false, zeroMove: Boolean = false) = when (dimensions) {
            2 -> {
                (-1..1).product(-1..1)
     ...
     */
    fun neighbours(moves: Iterable<Vector>, limits: List<Iterable<Int>>? = null, wrap: Boolean = false) =
        sequence<Position> {
            moves.forEach {
    ...
    fun wrapToLimits(limits: List<Iterable<Int>>): Position {
        ...
        val data = (position.data zip limits).map { (coordinate, limit) ->
    ...
    private fun isInLimits(limits: List<Iterable<Int>>): Boolean {
        check(limits.size == this.size) { "Size of limits must match size of Position" }
